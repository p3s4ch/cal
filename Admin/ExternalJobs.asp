...
<h2 ALIGN=CENTER>External Jobs </h2>
...
SQL = "SELECT * from vExternalJobs "   &  SQL_Where & " order by [date] desc"
SQL_TOT = "SELECT  IsNull( sum( IsNull(Billtotal,0)),0) MonthTotal from vExternalJobs " &  SQL_Where
...
MonthTotal = RS_TOT("MonthTotal")

%>
<font color=blue size=3>Total for vExternalJobs excluding materials in <%= MonthName(iMonth) %>  &pound; <%=MonthTotal %> </font>
