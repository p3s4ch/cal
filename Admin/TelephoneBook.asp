...
<!--#include file="common/update_generic2.asp"-->
...
Response.Write "<br> <H2> Telephone Book </H2> <br>"
%>


<form action="TelephoneBook_add.asp" name="frmUser" Method="POST">
...
<%
If (DeleteID <> "") Then
  SQL = "update TelephoneBook set isDeleted=1 WHERE ID = " & DeleteID
...
Else
	SQL_SearchFor = " WHERE isDeleted <> 1 "
End If
...
SQL = "SELECT * FROM TelephoneBook " & SQL_SearchFor & SQL_OrderBy
Response.Write SQL

%>
<form action="?" Method="GET"><!--#include file="common\searchForm.asp"--></form>
...
<a href="telephoneBook_add.asp?uid=<%=uid%>&ID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Edit</a>&nbsp;&nbsp;
<a href="?uid=<%=uid%>&DeleteID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Remove</a></font></td>

<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("ID")   %></font></td>
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1"><%=Left(RS("Surname"),20)   %></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=Left(RS("FirstName"),20)%></font></td>
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1"><%=Left(RS("Address"),20)%></font></td>

<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("Phone")%></font></td>
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1"><%=Left(RS("Comment"),50)%></font></td>
