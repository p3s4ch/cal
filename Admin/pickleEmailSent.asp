...
chaseStatus=   Replace(Trim(request("chaseStatus")), ";","")
chaseID =      Replace(Trim(request("chaseID")), ";","")
chaseIDColumn =      Replace(Trim(request("chaseIDColumn")), ";","")
chaseTable =   Replace(Trim(request("chaseTable")), ";","")


if chaseIDColumn = "" then
  chaseIDColumn = "id"
end if
sql="update "& chaseTable &" set Status='"& chaseStatus &"' where "& chaseIDColumn & "= "& chaseID

Response.Write sql
Response.Write "<br>"

Set RS_TOT = SQLCN.Execute(sql)
...
<a href="main.asp?uid=<%=uid%>" > Back Home </a>
