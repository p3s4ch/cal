...
<td bgcolor="#FFFFFF" width="89%" valign="top">
 ...
 Previous comments: <%=prevFeedback%>
<%
...
<%
  if (conf = "Unpaid" or conf = "Not Paid" or conf = "Requires Feedback" ) and not isCustomer=1  then
%>

   <form action="FeedbackThanks.asp" name="frmUser2" Method="GET">
     <input type="hidden" name="uid" value=<%=uid%> />
     <input type="hidden" name="EventID" value=<%=AddID%> />
   Materials & Labour :
   <textarea name="mat_lab" class="inputbox" cols="60" rows="2">  </textarea> <br>

   Customer already paid &pound; <input type="text" name="CustomerAlreadyPaid" class="inputbox" size="10" maxlength="255"> <br>
   AND / OR
...
%>
    ...
    <input type="submit"  color ="red" name="Submit"
              value="Click here if contactor only has been paid !" class="inputSubmit"></form>
...
<input type="hidden" name="eventID" value=<%=AddId%> />
&nbsp; &nbsp; &nbsp;
<textarea name="feedback" class="inputbox" cols="60" rows="2">  </textarea>
