...
<h2 ALIGN=CENTER>Channel Book ..</h2>



<form action="ChannelBook_add.asp" name="frmUser" Method="GET">
...
<!--#include file="common\generic_sql_delete.asp"-->
<!--#include file="common\update_channelBook.asp"-->
...
If NOT (SearchFor = "") Then
  SQL_SearchFor = " WHERE " &_
    ...
    " Channel LIKE ('%" & SearchFor & "%') or " &_
    " Logins LIKE ('%" & SearchFor & "%') or " &_
    " Contacts LIKE ('%" & SearchFor & "%') or " &_
    " Fees LIKE ('%" & SearchFor & "%') or " &_
    " Comment LIKE ('%" & SearchFor & "%') or " &_
    " Audit LIKE ('%" & SearchFor & "%') "
End If

If orderBy      = "" Then
	SQL_OrderBy = " ORDER BY Channel"
End If

SQL_Where = SQL_SearchFor

SQL = "select ID ,  Channel,  Logins , Status  from dbo.ChannelBook " &  SQL_Where  & SQL_OrderBy
...
SQL_TOT = "select count(1) as tot  from dbo.ChannelBook " &  SQL_Where

Set RS_TOT = SQLCN.Execute(SQL_TOT)
tots = RS_TOT("tot")
Response.Write "<br><br> <b>Total &nbsp;  " & tots & "</b><br>"

'if tots <> "" then
  'Response.Write "<br><br> <b>Total &nbsp;  " & FormatNumber(tots) & "</b><br>"
'end if
%>
<form  Method="GET">
   <br>
...
<%
addEditPgLink="ChannelBook_add.asp"
for each x in RS.Fields
%>
