...
<h2 ALIGN=CENTER>InvoiceEvents </h2>



<form action="InvoiceEvents_add.asp" name="frmUser" Method="GET">
...
<!--not include file="common\update_InvoiceEvents.asp"-->
...
If NOT (SearchFor = "") Then
     SQL_SearchFor = " AND (" &_
	" ie.InvoiceID LIKE '%" & SearchFor & "%' or " &_
	" ie.CustomerID LIKE '%" & SearchFor & "%' or " &_
	" c.Surname LIKE '%" & SearchFor & "%' or " &_
	" ie.EventID LIKE '%" & SearchFor & "%' " &_
	")"
...
SQL = "select ie.* , c.Surname, e.Confirmed from dbo.InvoiceEvents ie , cEvents e , Customers c where  ie.EventID=e.ID and ie.CustomerID=c.ID " &  SQL_Where  & " order by InvoiceID desc"
Response.Write "<br>" & SQL & "<br>"

Response.Write "Total //TODO: <br> <br>"
...
<%
addEditPgLink="InvoiceEvents_add.asp"
for each x in RS.Fields
%>
