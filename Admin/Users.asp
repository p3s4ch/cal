...
<!--#include file="common\update_generic5.asp"-->

<form action="Users_add.asp" name="frmUser" Method="Get">
...
View All Users where Role : <!--#include file="common\drops\dropDownUserRole.asp"-->
...
If NOT (SearchFor = "") Then
	...
	" [Admin_ID] LIKE '%" & SearchFor & "%' or " &_
	" [Surname] LIKE '%" & SearchFor & "%' or " &_
	" [FirstName] LIKE '%" & SearchFor & "%' " &_
	")"

End If

SQL_Where = SQL_SearchFor
SQL = "select * from usersByrole ('ALL') " & SQL_Where


'if (filter="usersByrole" ) Then
    SQL = "select * from usersByrole ('" & role &"') " & SQL_Where
'END IF
