...
function GetMiniMonthAsRow(iMth,iYr,op)
  ...
  WHILE DayOfMonth <= totDays
        ...
        'morning = '25 August 2014'
        'dayPlan = "<br>"


        sql_step1 = " select count(1) sold from dbo.f_RoomsSoldBreakdown ( '"& morning &"', 'Bed in Dorm' , 0 )"

        Set RS2 = SQLCN.Execute(sql_step1)
        dayPlan = RS2("Sold")

        anc="<a href=WeekView.asp?uid="&uid&"&operator="&op&"&iDay="& DayOfMonth & "&iMonth="& m & "&iYear="& y &">"

        htmRow3 = htmRow3 & "<td>" & anc & "<font color=black><b>" & dayPlan & "</b></font></a></td>"
