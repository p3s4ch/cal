...
<%
   	...
   	if (AddId ="") or isNull(AddId) Then
	    AddId = "-1"
	    sql = "select top 1 -1 as ID, 100 as ChequeNo , getDate() as Date,'' as Beneficiary,'' as CostCode ,'' as Details, 0 as Amount, -1 as HSBC_TX from Cheques"
	else
	    sql = "select top 1 * from Cheques where ID = " &AddId
	end if
...
<form action="cheques.asp" name="frmUser" Method="GET">
