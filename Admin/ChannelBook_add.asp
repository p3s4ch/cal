...
<form action="ChannelBook.asp" name="Form" Method="POST">
<table border="0" cellpadding="3" cellspacing="0" align="center" width = 90%>
<%

If (AddID <> "") Then
	  sql = "select ID, Channel , Logins , Contacts, Status, Fees, Comment from ChannelBook where id = " & AddID
	Else
	  AddID=-1
	  sql = "select -1 ID, '' Channel , '' Logins , '' Contacts, '' Status, '' Fees, '' Comment"
End if
...
if isAComment then
 %>
  ...
  <td bgcolor="<%=clr%>" > </td>
  <td bgcolor="<%=clr%>" ><textarea name="<%=x.Name%>" class="inputbox" cols="80" rows="20" ><%=x.Value%> </textarea></td>
</tr>
<%
end if
...
<input type="hidden" name="tableName" value="ChannelBook" />
