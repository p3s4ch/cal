...
<h2 ALIGN=CENTER>All Items by Codes and Categories based on Bank Transactions </h2>
...
If   (CostCodeCat <> "") Then
     SQL_SearchFor = " AND  CostCode in (select CostCode from CostCodeCat where Cat='"& CostCodeCat &"') "
End If
...
SQL = "select SlipNo,SlipNo Slip, [Date],CostCode,Supplier,Description, Total, Scan, Payment, TxID as Tx from Costs " & SQL_Where & " order by date"


if doGroupByCC="1" then
   SQL = "select CostCode, IsNull(sum(IsNull(Total,0)),0)  as total from Costs " &  SQL_Where & " group by CostCode order by 1 "
end if
...
SQL_totExcl  =  "select IsNull(sum(IsNull(Total,0)),0)  as tot from Costs " &  SQL_Where
Set RS_TOTexcl = SQLCN.Execute(SQL_totExcl)
tot = FormatNumber(RS_TOTexcl("tot"), 2)
if isDebug then
  Response.Write "SQL_totExcl="& SQL_totExcl & "<br>"
end if




Response.Write "<b>Total  &nbsp;&pound;" & tot & " This figure exludes: <b>"
Response.Write "<br>"
%>
 <!-- do not include file="common\CostCodesExcludes.asp"-->
