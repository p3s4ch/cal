...
If orderBy      = "" Then
	SQL_OrderBy = " ORDER BY DateReceived DESC"
...
<% 
dt = CDate(RS("DateReceived"))
dtStr=Day(dt) & " "& MonthName(Month(dt)) & " "& Year(dt)

%> 
</td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("ID")   %></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=dtStr%></font></td>
