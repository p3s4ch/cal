...
<h2 ALIGN=CENTER>Costs and Sale Codes</h2>
...
SQL = "select CostCode, cast (Sum(Total) as money) [Combined Total] from Costs "& SQL_WhereBetween &" AND CostCode not like '%~%' group by CostCode order by 1 "
end if


SQL_totExcl  =  "select IsNull(sum(IsNull(Total,0)),0)  as tot from Costs " &  SQL_Where & " AND CostCode not like '%~%'"
Set RS_TOTexcl = SQLCN.Execute(SQL_totExcl)
tot = FormatNumber(RS_TOTexcl("tot"), 2)
Response.Write "<b>Total  &nbsp;&pound;" & tot & " This exludes: <b>"
Response.Write "<b>This exludes: <b><br>"
%>
 <!--#include file="common\CostCodesExcludes.asp"-->
