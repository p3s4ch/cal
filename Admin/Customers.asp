...
<form action="customers.asp" name="frmUser" Method="Get">
  <input type=hidden value=<%=uid%> name=uid >
  <input type=hidden value=2 name=ReqInv > &nbsp;
  <input type="submit" name="Submit" value="filter.ReqInv2.micro" class="inputSubmit">
</form>

<form action="customers.asp" name="frmUser" Method="Get">
  <input type=hidden value=<%=uid%> name=uid >
  <input type=hidden value=3 name=ReqInv > &nbsp;
  <input type="submit" name="Submit" value="filter.ReqInv3.good" class="inputSubmit">
</form>


<form action="ActionCustomers.asp" name="frmUser" Method="Get">
  <input type=hidden value=<%=uid%> name=uid >
  <input type=hidden value=3 name=ReqInv > &nbsp;
  <input type="submit" name="Submit" value="ActionCustomers 2 do" class="inputSubmit">
</form>


<%

  Set RSTOT = SQLCN.Execute("select count(1) tot from Customers")
  Response.Write "Total Customer = " & RSTOT("tot") & "<br>"
...
if ReqInv <> "" Then
   SQL_ReqInv=" WHERE ReqInv=" & ReqInv
   if SQL_SearchFor <> "" Then
       SQL_ReqInv=" AND ReqInv=" & ReqInv
   end if
end if



SQL = "SELECT * FROM Customers " &  SQL_SearchFor &   SQL_ReqInv & SQL_OrderBy
Response.Write SQL

%>
<form action="customers.asp" Method="GET"><!--#include file="common\searchForm.asp"--></form>
...
<td bgcolor="#DAE3F0" align="center"><font face="Verdana, arial" size="1" color="#000000"><b>
<a href="?orderby=Phone_Up&page=<%If NOT (orderby = "Phone_Up") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">-</a>&nbsp;
Heard About&nbsp;<a href="?orderby=Phone_Down&page=<%If NOT (orderby = "Phone_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
<a href="customers_add.asp?uid=<%=uid%>&ID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Edit</a>&nbsp;&nbsp;
<a href="customers.asp?uid=<%=uid%>&DeleteID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Remove</a></font></td>
...
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("HeardAboutUs")%></font></td>
...
<%If cInt(intPage) > 1 Then%>
	<a href="?uid=<%=uid%>&orderby=<%=orderBy%>&page=<%=intPage - 1%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>"><< <b>Prev</b></a>
<%End If%>
</font></td>
<td width="50%" align="right"><font face="Verdana, arial" size="1" >
<%If cInt(intPage) < cInt(intPageCount) Then%>
	<a href="?uid=<%=uid%>&orderby=<%=orderBy%>&page=<%=intPage + 1%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>"><b>Next</b> >></a>
<%End If%>
