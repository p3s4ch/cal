...
%>
      <!--not include file="common\update_reservation.asp"-->
<%
End If
%>
...
SQL_ARRIVALS = " select id2print, id2edit,Reservation ,Surname, [GBP], Arrival , nights , RoomNum, Status , GuestRequests " &_
               " from vReservationDetailRooms "
SQL_ARRIVALS_PRINT = " select Reservation , Source , Arrival , nights , RoomType ,RoomNum, NR, Status " &_
               " from vReservationDetailRooms "
...
if (filterDays="groupByMultiLets" ) Then
    SQL = "select Reservation, count(1) [Total Number Of Rooms] from dbo.vReservationDetailRooms Group By Reservation having count(1) > 1"
END IF


if (filterDays="CheckinsT7" ) Then
    SQL_WHERE = " where [Arrival] BETWEEN '" & betweenFrom &"'  AND  '" & betweenTo & "' "
    SQL = SQL_ARRIVALS & SQL_WHERE
    SQL_TOT = SQL_TOT & SQL_WHERE
    print_sql = SQL_ARRIVALS_PRINT & SQL_WHERE
END IF

if (filterDays="CheckinsT1" ) Then
    SQL_WHERE = " where [Arrival] = dbo.truncDate(getDate()) "
    SQL = SQL_ARRIVALS & SQL_WHERE
    SQL_TOT = SQL_TOT & SQL_WHERE
