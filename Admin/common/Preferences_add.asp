...
<form action="Preferences.asp" name="frmUser" Method="GET">
<table border="0" cellpadding="3" cellspacing="0" align="center" width = 90%>
<%
set RS = SQLCN.Execute("select top 1 * from dbo.Preferences where ID = " & tempID )
...
'if (InStr(x.Name,"No") > 0) and (x.Name <> "ReceiptNo")  then isAnID =true else isAnID=false end if
...
if isPayment then
 %>
  <tr>
  <td bgcolor="<%=clr%>" ><font face="Verdana"  color="48576C" size="1">  <b>Payment</b></font></td>
  <td bgcolor="<%=clr%>" > </td>
  <td bgcolor="<%=clr%>" >
                          <select name="payment" class="inputBox">
                        <option value="" SELECTED></option>
                        <%
                        Set RS_CC= Server.CreateObject("ADODB.Recordset")
                        RS_CC.Open "select distinct Payment from Costs order by 1", SQLCN, 2, 2
	                        Do While Not RS_CC.eof
	                        if RS("Payment") = RS_CC("Payment") then
	                        %>
		                        <option value="<%=RS_CC("Payment")%>" selected><%=RS_CC("Payment")%></option>
	                        <%
	                        else
	                        %>
	                        <option value="<%=RS_CC("Payment")%>"><%=RS_CC("Payment")%></option>
	                        <%
	                        end if
                        RS_CC.movenext
                        Loop
   ...
   <font face="Verdana"  color="48576C" size="1">  Other </font>
   <input type="text" name="paymentOther" class="inputbox" size="25" maxlength="255" ></td>
</tr>
<%
end if
...
if Not isADateType AND NOT isAnID and NOT isCostCode and NOT isPayment then
 %>
  <tr>
  <td bgcolor="<%=clr%>" ><font face="Verdana"  color="48576C" size="1">  <b><%=x.Name%> </b></font></td>
  <td bgcolor="<%=clr%>" ></td>
