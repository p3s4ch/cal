...
SQL_SELECT_OPS_4EDIT =" select    ID, ' "& Admin_ID &"' enteredBy_hidden , '' Operator_Ext , getDate() as AsOfEntryTime_Ext,0 Amount, '' Comment , '' PUN_Ext "
...
if filter_on="AgentsAccount" then
   ...
   SQL = "select  ID id2edit , Date , Comment , Amount from fees where payer= '"& pm2 &"' order by Date desc"
...
if filter_on="OperatorsAccount" then
   ...
   next_bttns="OperatorAccountAdmin.Buttons"
   SQL = "select  ID id2edit , Operator , JobDate , OperatorFee , Comment, Client_PUN PUN, EnteredBy from dbo.OperatorAccount where Operator= '"& pm2 &"' order by JobDate desc"
...
fieldName="pm2"
%>
Agent Account: <!--#include file="dropDownPickleCashAccounts.asp"-->
