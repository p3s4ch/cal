...
<!--#include file="common/update_TelephoneBook.asp"-->
...
If (DeleteID <> "") Then
  SQL = "DELETE FROM Advertisers WHERE ID = " & DeleteID
...
'SQL = "SELECT * FROM Advertisers " & SQL_SearchFor & SQL_OrderBy
...
Company Name&nbsp;<a href="?orderby=Surname_Down&page=<%If NOT (orderby = "Surname_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
Contact(s)&nbsp;<a href="?orderby=Name_Down&page=<%If NOT (orderby = "FirstName_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
Telphone&nbsp;<a href="?orderby=Address_Down&page=<%If NOT (orderby = "Address_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
Comment&nbsp;<a href="?orderby=PostCode_Down&page=<%If NOT (orderby = "PostCode_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
<a href="Advertisers_add.asp?ID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Edit</a>&nbsp;&nbsp;
<a href="Advertisers.asp?DeleteID=<%=RS("ID")%>&orderby=<%=orderby%>&page=<%=intPage%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">Remove</a></font></td>

<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("ID")   %></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("Company_Name")   %></font></td>
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1"><%=RS("Contact")%></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1"><%=RS("Telephone")%></font></td>
