...
<%
   	...
   	if (AddId ="") or isNull(AddId) Then
	    AddId = "-1"
	    tempID = 212
	    sql ="select top 1 ID,Payer, getDate() as Date,Amount,Comment from fees where ID = " & tempID
	else
	    sql ="select top 1 ID,Payer, Date,Amount,Comment from fees where ID = " & AddId
