...
<h2 ALIGN=CENTER>HSBC Business Card</h2>
...
SQL_Where = SQL_WhereBetween & " and Holder like '%ronit%' "
...
SQL_totDeposits  = "SELECT IsNull(sum(IsNull(Amount,0)),0) as tot FROM Fees " & SQL_WhereBetween & " and [name] like '%p_buscard%' "
SQL_totPayments  = "SELECT IsNull(sum(IsNull(Amount,0)),0) as tot FROM Payment " & SQL_WhereBetween & " and [name] like '%p_buscard%' "
SQL_totWithdraws= "SELECT  IsNull(sum(IsNull(Amount,0)),0)  as tot FROM HSBC_BUSCARD " & SQL_Where
'Response.Write "<br>" & SQL_totDeposits & "<br>"


Set RS_TOT_DEPOSITS = SQLCN.Execute(SQL_totDeposits)
Set RS_TOT_PAYMENTS = SQLCN.Execute(SQL_totPayments)
Set RS_TOT_WITHDRAWS = SQLCN.Execute(SQL_totWithdraws)

tot_d = FormatNumber(RS_TOT_DEPOSITS("tot"), 2)
tot_p = FormatNumber(RS_TOT_PAYMENTS("tot"), 2)
tot_w =  FormatNumber(RS_TOT_WITHDRAWS("tot"),2)


Response.Write "<br><b>Total Card &nbsp;&pound;" & tot_w & "<b><br>"
Response.Write "<br><b>Total Paid &nbsp;&pound;" & tot_d & "<b><br>"
Response.Write "<br><b>Micro Men &nbsp;&pound;" & tot_p & "<b><br>"
...
if tot > 0 then
  accountStatus="<font color =red> Card not yet paid </font>"
else
  accountStatus="<font color =green> Card is Fully Paid </font>"
end if

Response.Write "<b><br>Otstanding &nbsp;&pound;"    & FormatNumber(tot,2) & "<b> &nbsp; "& accountStatus &" <br>"
