...
function GetMiniMonthAsRow(iMth,iYr,op)
 ...
 innerTable="<table class=class3 border=2 cellpadding=0 cellspacing=1 width=100% > "
  DayOfMonth = 1
  ...
  WHILE DayOfMonth <= totDays
    	...
    	if today=ViewDt then cc="c_weekday1" end if
  		...
  		if iWkday = 1 then
		  		    htm5= htm5 & "<td class="& cc & ">  S"& anc & DayOfMonth & "</a> "& cats &"</td>"
		end if

  		if iWkday = 2 then
		  		    htm5= htm5 & "<td class="& cc & ">  M"& anc & DayOfMonth & "</a> "& cats &"</td>"
		  		end if

		if iWkday = 3 then
				  	htm5= htm5 & "<td class="& cc & ">  T"& anc & DayOfMonth & "</a> "& cats &"</td>"
		end if

		if iWkday = 4 then
		  		    htm5= htm5 & "<td class="& cc & ">  W"& anc & DayOfMonth & "</a> "& cats &"</td>"
		  		end if

		if iWkday = 5 then
				  	htm5= htm5 & "<td class="& cc & ">  Th"& anc & DayOfMonth & "</a> "& cats &"</td>"
		end if

        if iWkday = 6 then
				  	htm5= htm5 & "<td class="& cc & ">  F"& anc & DayOfMonth & "</a> "& cats &"</td>"
		end if
