...
<h2 ALIGN=CENTER>Periods </h2>



<form action="Periods_add.asp" name="frmUser" Method="GET">
...
<!--#not include file="common\update_Periods.asp"-->
...
If NOT (SearchFor = "") Then
     SQL_SearchFor = " AND (" &_
	" [PeriodStart] LIKE '%" & SearchFor & "%' or " &_
	" [Name] LIKE '%" & SearchFor & "%' or " &_
	" [Cat] LIKE '%" & SearchFor & "%' " &_
	")"
...
SQL = "select * from dbo.Periods " &  SQL_Where  & " order by [PeriodStart] desc"
