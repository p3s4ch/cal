...
<h2 ALIGN=CENTER>Report 3: Sales </h2>
...
sql_p = "dbo.pInvoices  '"& betweenFrom & "','" & betweenTo &"', 'UNPAID'"
SQLCN.Execute(sql_p)
'Response.Write "<br>" & sql_p &"<br>"
'Response.End
SQL = "select * from dbo.TmpInvoices3"


SQL_tot  =  "select IsNull(sum(IsNull(Total,0)),0)  as tot from dbo.TmpInvoices3 "
Set RS_TOT = SQLCN.Execute(SQL_tot)
tot = FormatNumber(RS_TOT("tot"), 2)
Response.Write "<br><b>Total Sales excl VAT For This Period&nbsp;&pound;" & tot & "<b><br>"



SQL_tot  =  "select IsNull(sum(IsNull(VAT,0)),0)  as tot from dbo.TmpInvoices3 "
Set RS_TOT = SQLCN.Execute(SQL_tot)
tot = FormatNumber(RS_TOT("tot"), 2)
Response.Write "<br><b>Total VAT Received For This Period&nbsp;&pound;" & tot & "<b><br>"


SQL_tot  =  "select IsNull(sum(IsNull(TotalInclVAT,0)),0)  as tot from dbo.TmpInvoices3 "
Set RS_TOT = SQLCN.Execute(SQL_tot)
tot = FormatNumber(RS_TOT("tot"), 2)
Response.Write "<br><b>Total Incl VAT For This Period&nbsp;&pound;" & tot & "<b><br>"


SQL_tot  =  "select IsNull(sum(IsNull(UnPdAsOf,0)),0)  as tot from dbo.TmpInvoices3 where UnPdAsOf > 0 "
Set RS_TOT = SQLCN.Execute(SQL_tot)
tot = FormatNumber(RS_TOT("tot"), 2)
Response.Write "<br><b>Total Unpaid As Of "& betweenTo &" &nbsp;&pound;" & tot & "<b><br>"
