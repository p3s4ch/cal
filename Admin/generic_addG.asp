...
<%
   	AddID = request("ID")
	If (AddID <> "") Then myID =AddID else myID = 1 end if
...
<%
set RS = SQLCN.Execute("select top 1 * from " & tableName  &  " where ID ="& myID)
...
if isAnID then
 %>
  ...
  <td bgcolor="<%=clr%>" >  </td>
  <td bgcolor="<%=clr%>" ><input type="text" name="<%=x.Name%>" value="<%=x.Value%>" class="inputbox" size="15" maxlength="255"></td>
</tr>
<%
end if
