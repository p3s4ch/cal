...
If NOT (SearchFor = "") Then
	...
	" i.ID LIKE ('%" & SearchFor & "%') or " &_
	" i.Description LIKE ('%" & SearchFor & "%') or " &_
	" i.DateOfInvoice LIKE ('%" & SearchFor & "%') or " &_
	" i.TotalInclVAT LIKE ('%" & SearchFor & "%') or " &_
	" i.Cat LIKE ('%" & SearchFor & "%') or " &_

	" c.FirstName LIKE ('%" & SearchFor & "%') or " &_
	" c.Surname LIKE ('%" & SearchFor & "%') or " &_
...
Else
	SQL_SearchFor = " WHERE Description <> 'Cancelled'"
End If


If orderBy      = "" Then
	SQL_OrderBy = " ORDER BY i.ID DESC"

ElseIf orderBy      = "ID" Then
	SQL_OrderBy = " ORDER BY i.ID"
...
SQL = "SELECT i.ID No, * FROM Invoices i left join Customers c on i.CustomerID = c.ID"  & SQL_SearchFor & SQL_OrderBy
'Response.Write SQL

%>
<form action="Invoices.asp" Method="GET"><!--#include file="common\searchForm.asp"--></form>
...
<a href="?orderby=TotalInclVAT_Up&page=<%If NOT (orderby = "TotalInclVAT_Up") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">-</a>&nbsp;
 &nbsp;  &nbsp; &pound; &nbsp;<a href="?orderby=TotalInclVAT_Down&page=<%If NOT (orderby = "TotalInclVAT_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>

<td bgcolor="#DAE3F0" align="center"><font face="Verdana, arial" size="1" color="#000000"><b>
<a href="?orderby=Cat_Up&page=<%If NOT (orderby = "Cat_Up") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">-</a>&nbsp;
Status&nbsp;<a href="?orderby=Cat_Down&page=<%If NOT (orderby = "Cat_Down") Then%>1<%Else%><%=intPage%><%End If%>&SearchFor=<%=SearchFor%>&SearchWhere=<%=SearchWhere%>">+</a></b></font></td>
...
<a href="invoice_print.asp?ID=<%=RS("No")%>"> <font face="Verdana, arial" size="1" color=green>Preview</font> </a>  &nbsp;
<a href="invoices_add.asp?ID=<%=RS("No")%>">  <font face="Verdana, arial" size="1">Edit	</font></a>&nbsp;
<a href="revenue_add.asp?InvoiceID=<%=RS("No")%>">  <font face="Verdana, arial" size="1" color=blue>Payment</font></a>&nbsp;
...
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1"><%=RS("No")   %></font></td>
...
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1" color=<%=fontcolor2%> ><%=RS("Cat")%></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1" color=<%=fontcolor2%> ><%=RS("DateOfInvoice")%></font></td>
<td bgcolor="<%=color2%>"><font face="Verdana, arial" size="1" color=<%=fontcolor2%> ><%=FormatNumber(RS("TotalInclVAT"), 2)%></font></td>
<td bgcolor="<%=color1%>"><font face="Verdana, arial" size="1" color=<%=fontcolor%> > <b> <%=Left(RS("Status"),8)%> </b></font></td>
</tr>
