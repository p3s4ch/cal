...
<form action="HSBCimport.asp" name="frmUser2" Method="GET">
<input type="hidden" name="uid" value=<%=uid%> />
&nbsp; &nbsp; &nbsp;
<input type="submit" name="Submit" value="import from HSBC " class="inputSubmit">
</form>

<form action="UpdateCostsFromCheques.asp" name="frmUser2" Method="GET">
<input type="hidden" name="uid" value=<%=uid%> />
&nbsp; &nbsp; &nbsp;
<input type="submit" name="Submit" value="Update Costs From Cheques " class="inputSubmit">
</form>
...
SQL = "select * from Cheques " &  SQL_Where  & " order by [date] desc"
...
if isAnID then
 %>
   <td bgcolor="<%=color1%>">
   <a href=cheques_add.asp?uid=<%= uid %>&ID=<%= x.value %>><font face="Verdana, arial" size="1" color=red>edit</font></a></td>
 <%
 end if
 if Not isAnID then
 %>
