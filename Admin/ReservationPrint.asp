...
mysql = request("mysql")
mysql = "select RoomType from dbo.vReservationDetailRooms where Booking = '" & booking & "'"

booking = request("booking")
myheader = request("myheader")
mytotal=request("mytotal")
...
%>
   <!--#include file="common\banner2Print.asp"-->
...
<%
     ...
     sql_print = "select firstName , *  from dbo.vReservationDetailRooms where Reservation = '" & booking & "'"
     RSPRINT = SQLCN.Execute(sql_print)
     firstName= RSPRINT("firstName")
     surname = RSPRINT("surname")
     Status = RSPRINT("Status")

     Status = RSPRINT("Status")

        Response.Write "<br>.Guest Name.." & firstName & " "  & surname
	Response.Write "<br>" & Status
	Response.Write "<br>.Guest Name.." & firstName
        Response.Write "<br>.Guest Name.." & firstName

%>
<img  height="150" width="150" src="LogoOakGuest.jpg" >
...
'Response.Write "<br><br> <b>Total &nbsp; " & mytotal & "</b><br>"
...
%>
<tr>
<td colspan="5"><table border="1" cellpadding="1" cellspacing="1" width="100%">
<tr>
